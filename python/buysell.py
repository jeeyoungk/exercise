...
def buy_sell(raw_prices, k):
    ...
    mins = min_since_2(prices, maxes)
    # normalization gives a nice property, where
...
# def min_since(array): return partial_reduce(array, min)

def min_since_2(array, maxes):
    "similar to min-since, except it gets reset whenever max value changes"
    v = None
    output = []
...
def normalize(prices):
    """
    1. reduce consecutive increase / decrease in prices to a single increase
...
def main():
    ...
    print buy_sell([1,5,2,3], 2) == 5     # double range
    print buy_sell([1,3,2,4], 2) == 4     # range splitting
    ary = [100,140,50,71,60,75,70,80]
    print buy_sell(ary, 1) == 40
    print buy_sell(ary, 2) == 70
    print buy_sell(ary, 3) == 81
