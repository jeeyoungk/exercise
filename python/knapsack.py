...
def knapsack(size, inputs):
    ...
    for cur_input in inputs:
        # taking a copy, as we're modifying the dictionary
        # as we are iterating.
        for prev_value, prev_history in history.items():
...
def knapsack_wrapper(size, inputs):
    ...
    if result is None:
        print "%d is not possible from combining %s" % (size, ", ".join(map(str, inputs)))
    else:
        print "%d = %s" % (size, " + ".join(map(str, result)))
